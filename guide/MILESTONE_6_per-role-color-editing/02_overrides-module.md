...
1. **Is this even a color?** The hexes arriving here were typed by a person into a text box inside a webview.
   `banana`, `#f00`, `rgb(1,2,3)` and an empty string are all things you will produce. The engine has no
...
> **Before you start:** step 01 is saved and compiling. (This file imports only `Hex` from `./types`;
> `ThemeOverrides` is what step 03 hands to these functions, one group at a time.)
...
2. **Import the two types it needs** and declare the hex pattern. `HEX_6` is module-level and `SCREAMING_SNAKE`
   ...
   `#aabbcc`, because `<input type="color">` emits lowercase while you, typing by hand, often won't.
...
4. **Add `overrideRoles`** below `isHex`. This is the merge — six lines, and the only place a hand-picked color
   ...
   ```
5. That's the whole file — **two functions, nothing else.** Resist adding a helper you can't call yet: everything
   here is consumed by [step 03](03_generate-overrides.md)'s three merge points, and a third exported function with
   no caller would be dead weight the compiler can't warn you about.
...
`<input type="color">` in step 06 will silently refuse the value); the exported names `isHex` and `overrideRoles`
(step 03 imports `overrideRoles`; `isHex` is used by `overrideRoles` itself and by the check below). The file
**path** `src/engine/overrides.ts` is load-bearing for the `node -e` check and the imports. Everything else —
comment wording, the `HEX_6` constant name — is cosmetic.

## Done when (this step)
- `src/engine/overrides.ts` exists with the two exported functions (`isHex`, `overrideRoles`) and `npm run compile` is clean.
